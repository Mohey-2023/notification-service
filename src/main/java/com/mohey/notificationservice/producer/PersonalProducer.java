...
import com.mohey.notificationservice.dto.FCMNoticeDto;
...
public class PersonalProducer {
    ...
    public FCMNoticeDto send(String topic, FCMNoticeDto fcmNoticeDto) {
        log.info("topic : " + topic);
        ObjectMapper mapper = new ObjectMapper();
        String jsonInString = "";
        try {
            jsonInString = mapper.writeValueAsString(fcmNoticeDto);
            log.info("json : " + jsonInString);
        } catch (JsonProcessingException ex) {
            ex.printStackTrace();
        }
        kafkaTemplate.send(topic, jsonInString);
        log.info("fcm 푸시 서비스에 데이터를 보냈습니다 : " + fcmNoticeDto);
        return fcmNoticeDto;
    }
}
